...
async function obtenerRespuestaAPI(pregunta, callback) {
  ...
  const body = JSON.stringify({
    ...
    messages: [
      {
        role: "system",
        content: "Eres un asistente coqueto y atrevido",
      },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function mostrarHistorial() {
    ...
    if (historial.length === 0) {
      chatMessages.innerHTML =
        '<div style="color:#888;text-align:center;margin:2em 0;">No hay conversaciones previas.</div>';
      return;
    }
    historial.forEach((msg) => {
  ...
  function agregarMensajeAlDOM(autor, contenido) {
    ...
    div.innerHTML = `
      <div class="upperChat">
        <img class="upperChat__userImage${
          autor !== "User" ? " upperChat__userImage--ai" : ""
        }"
          src="${
            autor === "User"
              ? "./assets/icons/messages/user-svgrepo-com.svg"
              : "./assets/icons/sideBar/share-circle-svgrepo-com.svg"
          }" alt="" />
  ...
  function enviarMensaje() {
    ...
    obtenerRespuestaAPI(texto, function (respuesta) {
      ...
      if (!respuesta || respuesta.trim() === "") {
        agregarMensajeAlDOM(
          "AI",
          "No se recibió respuesta de la IA. Intenta de nuevo o revisa la consola para más detalles."
        );
      } else {
  ...
  chatMessages.innerHTML =
    '<div style="color:#888;text-align:center;margin:2em 0;">¡Empieza una nueva conversación!</div>';
